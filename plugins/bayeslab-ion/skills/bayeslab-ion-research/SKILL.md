...
1. Resolve and show the exact ticker, company name, industry slug, and optional
   notes.
2. Explain that the run creates a new isolated result.
...
`includeReport` omitted or false until the status is terminal. Set
`includeReport: true` only after completion and only when the full Markdown
report is needed.
...
tool output does not provide. Keep tool polling compact and avoid requesting a
full report repeatedly.
